...
func TestLatencyChecker_RunCommandExec(t *testing.T) {
	var input Core.LatencyAPIRequest
	var outputTokenOK = Core.TokenAPIResponse{
		RequestCount: 100000,
		Duration:     100,
	}
	var outputTokenKOInsufficient = Core.TokenAPIResponse{
		RequestCount: 1,
		Duration:     100,
	}

	var outputLatencyOK = Core.LatencyCheckerOutputList{
		Result: []Core.LatencyCheckerOutput{
			{
				Location:   "us-east-1",
				AvgLatency: 200,
			},
	...
	tests := []struct {
		name    string
		fields  fields
		want    Core.LatencyCheckerOutputList
		wantErr bool
	}{
		{
